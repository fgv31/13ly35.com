...
const categoryConfig: Record<string, { label: string; color: string; dotColor: string }> = {
	live: { label: "LIVE", color: "text-blue", dotColor: "bg-blue" },
	archived: { label: "ARCHIVED", color: "text-violet", dotColor: "bg-violet" },
	queued: { label: "QUEUED", color: "text-[#8A85A3]", dotColor: "bg-[#8A85A3]" },
};
	...
	function Timeline({ entries, onLocationClick }, ref) {
		...
		return (
			<div className="relative">
				{/* Vertical line */}
				<div className="absolute left-0 top-0 bottom-0 w-px bg-gradient-to-b from-blue/50 via-violet/50 to-white/10" />
							...
							>
								...
								<div
									className={`absolute left-[-5px] top-1 w-[10px] h-[10px] rounded-full transition-all duration-300 ${
										isExpanded
											? "bg-blue scale-150 shadow-[0_0_15px_var(--accent-blue)]"
											: cat.dotColor
									}`}
								/>
								...
								>
									<div className="flex items-baseline gap-4 mb-1">
										<span className="font-mono text-xs text-blue/60">
											{entry.period}
										</span>
										{entry.category === "live" ? (
											<span className="live-blink font-mono text-[10px] text-blue font-bold flex items-center gap-1.5">
												<span className="inline-block w-1.5 h-1.5 rounded-full bg-blue shadow-[0_0_8px_var(--accent-blue)]" />
												LIVE
											</span>
										) : (
									...
									<div className="flex items-baseline gap-2">
										<h3 className={`text-2xl font-bold transition-colors duration-300 ${
											isExpanded ? "text-blue glow-blue" : isQueued ? "text-white/50 group-hover:text-white/70" : "text-white group-hover:text-[var(--accent-cyan)]"
										}`}>
											{entry.location}
										</h3>
										{entry.country && (
									...
									<div className="relative ml-4 pl-6 border-l border-white/10 space-y-4">
										...
										{entry.link && (
											<div className="relative pt-2">
												<div className="absolute left-[-29px] top-[18px] w-[6px] h-[6px] rounded-full bg-blue/40" />
												<Link
													href={entry.link.href}
													className="inline-flex items-center gap-2 font-mono text-[10px] tracking-wider text-blue border border-blue/20 hover:border-blue hover:bg-blue/5 px-4 py-2 transition-all duration-300"
												>
													{entry.link.label}
